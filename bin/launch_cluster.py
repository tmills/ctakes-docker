...
UMLS_USER_KEY='ctakes_umlsuser'  # Needed if using cTAKES dictionary from sf.net or create your own using cTAKES GUI
...
BROKER_HOST_KEY='broker_host'
BROKER_PORT_KEY='broker_port'

# uima-as uses 61616 by default but 80 is more likely to be consistently open
broker_port=80
...
def main(args):
    ...
    if UMLS_USER_KEY in config_map.keys() and config_map[UMLS_USER_KEY]!='':
        umls_user = config_map[UMLS_USER_KEY]
    elif UMLS_USER_KEY in os.environ.keys():
        umls_user = os.environ[UMLS_USER_KEY]
    else: 
        umls_user = ""

    if UMLS_PW_KEY in config_map.keys() and config_map[UMLS_PW_KEY]!='':
        umls_pwd = config_map[UMLS_PW_KEY]
    elif UMLS_PW_KEY in os.environ.keys():
        umls_pwd = os.environ[UMLS_PW_KEY]
    else:
        umls_pwd = ""
    ...
    if USER_KEY_REQUIRED:
      if umls_user == "":
          ...
          sys.stderr.write("or the configuration file must set %s\n" % (UMLS_USER_KEY))
          sys.exit(-1)
      if umls_pwd == "":
          sys.stderr.write("The environment variable %s must be defined for this script\n" % (UMLS_PW_KEY))
          sys.stderr.write("or the configuration file must set %s\n" % (UMLS_PW_KEY))
    ...
    try:
        broker_instances = start_instances(ami=BROKER_AMI, 
                                        count=BROKER_REPLICATION, 
                                        instance_type=BROKER_MACHINE, 
                                        config=config_map, 
                                        broker_ip="localhost", 
                                        outfile=fout, 
    ...
    env_file = write_env_file({ ORCL_URL_KEY:config_map[ORCL_URL_KEY],
                                ...
                                BROKER_PORT_KEY:broker_port,
                                UMLS_USER_KEY:umls_user,
                                UMLS_PW_KEY:umls_pwd})
    ...
    try:
        start_containers(broker_instances, mist_instances, pipeline_instances, writer_instances, env_file, key_file)
...
def start_containers(broker_instances, mist_instances, pipeline_instances, writer_instances, env_file, key_file):
        ...
        if mist_instances is None:
            ...
            flag_out('Running mist container(s)')
        ...
        if pipeline_instances is None:
            ...
            flag_out('Running pipeline container(s)')
        ...
        if writer_instances is None:
            ...
            ## Start the database writer container on the writer instance(s), but don't start the 
            ## async service yet because we'll need to update the name of the table to write to
            ## after the container is started but before the async service is started.
            ## Otherwise we'd have to rebuild the container image with the table name before we 
            ## would start the container
            flag_out('Running writer container(s)')
            cmd = "cd ctakes-docker; ./bin/runWriterContainer.sh" 
...
def start_instances(ami, count, instance_type, config, broker_ip, outfile, name="Unnamed Instance", project="Unnamed Project"):
    ...
    """
    flag_out('Starting instance %s' % name)
...
def run_command_on_instances(instances, command, env_file, key_file, wait_time):
    ...
    for instance in get_instances(instances):
        ...
        out("Preparing to run command %s on instance %s\n" % (command, instance_id))
        ...
        if (wait_time > 0):
            ...
            for x in range(1, wait_time):   # now that the instance is running, delay to let ssh get started before we try even the first time
                time.sleep(1)
                out('.')
            print()
        ...
        # Use StrictHostKeyChecking=no to avoid "The authenticity of host 'ec2-xxx-yyy-zzz-www.compute-1.amazonaws.com (aaa.bbb.ccc.ddd)' can't be established."
        call(['scp', '-i', key_file, '-o', 'StrictHostKeyChecking=no', env_file.name, 'ubuntu@%s:/home/ubuntu/ctakes-docker/%s' % (inst_ip, ENV_FILE_NAME)]) 
        time.sleep(4)
        if log_details:
        ...
        #command='ls -la'
        #command='grep env *'
        out("Running the following command on a remote host %s:\n  %s\n" % (inst_ip, command))
        out("  Current time is %s\n" % (datetime.datetime.now().strftime(format)))
        out("  Command's output won't be displayed until after command finishes.\n")
        out("  If you need to stop the process and Control-C is not working, you can use Control-Z to stop.\n")
        stdin, stdout, stderr = ssh.exec_command(command)
        stdin.flush()
        output = stdout.read().decode('ascii').splitlines()
        error = stderr.read().decode('ascii').splitlines()
...
def wait_for_instances_to_run(instances):
    ...
    for instance in get_instances(instances):
        ...
        out("  Use Control-C to kill the process if needed ....")
